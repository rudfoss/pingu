...
import { getClient } from "./initAppInsights"
import { LogMessage } from "./LogMessage"

export interface AppInsightsTransportConstructorOptions
...
export class AppInsightsTransport extends TransportStream {
	...
	public constructor(
		public readonly options: AppInsightsTransportConstructorOptions = {}
	) {
		super(options)
	...
	public log(info: LogMessage, next: () => void) {
		...
		if (this.options.aiClient?.context.keys.operationId && aiOperationId) {
			tagOverrides[
				this.options.aiClient.context.keys.operationId
			] = aiOperationId
		}
